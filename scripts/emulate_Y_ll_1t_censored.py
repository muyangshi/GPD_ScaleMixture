"""
Feb. 20
Emulating the entire likelihood doesn't perform well
Suspect if it is between there are two pieces:
    - censored
    - exceedance
So let's train separate pieces for censored and exceedance
...
# exceedance_idx            = np.where(X_lhs[:,0] > X_lhs[:,1])[0]
# exceedance_idx_val        = np.where(X_lhs_val[:,0] > X_lhs_val[:,1])[0]
...
# censored_idx               = np.where(X_lhs[:,0] <= X_lhs[:,1])[0]
# censored_idx_val           = np.where(X_lhs_val[:,0] <= X_lhs_val[:,1])[0]
...
# np.save(rf'll_1t_X_exceed_{N}.npy', X_lhs_exceed)
# np.save(rf'll_1t_Y_exceed_{N}.npy', Y_lhs_exceed)
# np.save(rf'll_1t_X_val_exceed_{N_val}.npy', X_lhs_val_exceed)
# np.save(rf'll_1t_Y_val_exceed_{N_val}.npy', Y_lhs_val_exceed)

# np.save(rf'll_1t_X_censored_{N}.npy', X_lhs_censored)
# np.save(rf'll_1t_Y_censored_{N}.npy', Y_lhs_censored)
# np.save(rf'll_1t_X_val_censored_{N_val}.npy', X_lhs_val_censored)
# np.save(rf'll_1t_Y_val_censored_{N_val}.npy', Y_lhs_val_censored)
...
y_train     = np.exp(Y_lhs_censored)
y_val       = np.exp(Y_lhs_val_censored)
X_train     = X_lhs_censored
X_val       = X_lhs_val_censored
...
if unit_hypercube:
    ...
    np.save('X_min_censored.npy', X_min)
    np.save('X_max_censored.npy', X_max)
...
bestmodel.save(rf'./Y_L_1t_censored_NN_{N}.keras')
...
model_nn = keras.models.load_model(rf"Y_L_1t_censored_NN_{N}.keras")
X_min    = np.load('X_min_censored.npy')
X_max    = np.load('X_max_censored.npy')
