"""
A Separate emulator for the exceedance likelihood
"""
...
# l_bounds = [0.001,     5,  -1.0, 1e-2, -5.0, 0.05,       0.5,  1.0]
# u_bounds = [1000,     60,   1.0,  5e6,  5.0, 0.95,       8.0, 50.0]
...
# np.save(rf'll_1t_X_exceed_Y_minus_u_{N}.npy', X_lhs)
# np.save(rf'll_1t_Y_exceed_Y_minus_u_{N}.npy', Y_lhs)
...
# np.save(rf'll_1t_X_val_exceed_Y_minus_u_{N_val}.npy', X_lhs_val)
# np.save(rf'll_1t_Y_val_exceed_Y_minus_u_{N_val}.npy', Y_lhs_val)
...
# # trim the zeros
if trim_zeros:
    zero_idx = np.where(y_train == 0)
    X_train  = np.delete(X_train, zero_idx, axis = 0)
    y_train  = np.delete(y_train, zero_idx, axis = 0)

    zero_idx_val = np.where(y_val == 0)
    X_val  = np.delete(X_val, zero_idx_val, axis = 0)
    y_val  = np.delete(y_val, zero_idx_val, axis = 0)
...
bestmodel.save(rf'./Y_L_1t_exceed_Y_minus_u_NN_{N}.keras')
