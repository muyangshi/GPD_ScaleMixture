...
Marginal Parameter:
    ...
    Y                  - genpareto.ppf(0.999, c = 0.8, scale = 30)
...
# Y, scale, shape, R, Z, phi, gamma, tau
n_param = 8
...
u_bounds = np.array([scipy.stats.genpareto.ppf(0.999, c= 0.8, scale = 30), # Y
                     30.0,     #scale
                     0.8,   #shape
                     qlevy(0.95),  #R
                     5.0,   #Z
...
out_of_supp_idx = np.where(scipy.stats.genpareto.pdf(LHS_tmp[:,0], scale = LHS_tmp[:,1], c = LHS_tmp[:,2]) == 0)[0]
LHS_tmp         = np.delete(LHS_tmp, out_of_supp_idx, axis = 0)

n_samples = LHS_tmp.shape[0]
LHS_X     = np.column_stack((LHS_tmp[:,0], 
                             [0.9]*n_samples, 
                             [0.0]*n_samples, 
                             LHS_tmp[:,1:]))
