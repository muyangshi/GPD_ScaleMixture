...
def dRW_par(x, phi, gamma_bar, tau):
    return(dRW(x, phi, gamma_bar, tau))
...
with multiprocessing.get_context('fork').Pool(processes=n_processes) as pool:
    Y_lhs = pool.starmap(dRW_par, list(X_lhs))
...
start_time = time.time()
print('start calculating validation dRW()s:', datetime.datetime.now())
with multiprocessing.get_context('fork').Pool(processes=n_processes) as pool:
    Y_lhs_val = pool.starmap(dRW_par, list(X_lhs_val))
